...
void reachTargetX(){
  if (xCoordinate < targetXCoordinate - 1){
    forward1();
  }
  else if (xCoordinate > targetXCoordinate + 1){
    backward1();
  }
  else{
    reachTargetAngle();
...
void getAccelData(){
  ...
  pitch_a = 180 * atan2(X_prec*0.061, sqrt(Y_prec*0.061*Y_prec*0.061 + Z_prec*0.061*Z_prec*0.061))/M_PI; 
...
/********** Reach Target Angle********/
void reachTargetAngle(){
  while(1){
    getAccelData();
    if (pitch < targetAngle - 2){
      Serial.println("Less than target angle");
      analogWrite(en1, 0);
      delay(1);
      analogWrite(en2, mode2Speed);
      digitalWrite(ph2, HIGH);
    }
    else if (pitch > targetAngle + 2){
      Serial.println("Greater than target angle");
      analogWrite(en2, 0);
      delay(1);
      analogWrite(en1, mode2Speed);
      digitalWrite(ph1, HIGH);
